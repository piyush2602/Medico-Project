...
const callMLService = (symptoms) => {
  return new Promise((resolve, reject) => {
    ...
    const mlUrl =
      process.env.ML_SERVICE_URL || "https://medico-project-o7vv.onrender.com";
...
Respond ONLY with valid JSON in this exact format (no markdown, no code block):
{
  ...
  "confidence": <integer 0-100>
        ...
        console.log(
          "Gemini API error, trying ML service:",
          geminiError.message,
        );
      }
    }
    ...
    if (!recommendation) {
      try {
        ...
        if (
          mlResult.success &&
          VALID_SPECIALITIES.includes(mlResult.speciality)
        ) {
          recommendation = {
          ...
          console.log(
            `ML model predicted: ${mlResult.speciality} (${mlResult.confidence}%)`,
          );
        }
      } catch (mlError) {
        console.log(
          "ML service unavailable, using keyword fallback:",
          mlError.message,
        );
      }
    }
...
const analyzeReport = async (req, res) => {
  try {
    if (!req.file) {
      return res.json({
        success: false,
        message: "Please upload a PDF or image (JPG, PNG) medical report.",
      });
    }
    ...
    const buildPrompt = (
      content,
    ) => `You are a senior medical AI assistant analyzing a patient lab report.
...
Respond ONLY with valid JSON (no markdown, no code fences):
{
  ...
  "confidence": <integer 0-100>
    ...
    const geminiValid =
      geminiKey && geminiKey.length > 20 && !geminiKey.startsWith("your_");
    ...
    if (geminiValid) {
      try {
        ...
        if (extractedText.length >= 50) {
          ...
          geminiContents = [
            buildPrompt(
              "The attached PDF is the medical report. Read all content.",
            ),
          ...
          geminiContents = [
            buildPrompt(
              "The attached image is the medical lab report. Extract and analyze all visible values.",
            ),
        ...
        console.log(
          ...
          geminiErr?.status,
          geminiErr?.message?.substring(0, 80),
        );
      }
    }
    ...
    if (!analysis && extractedText.length >= 20) {
      try {
        ...
        if (mlResult.success) {
          ...
          engine = "ml";
          console.log(
            `ML analyzed report: ${mlResult.seriousness}, ${mlResult.speciality}`,
          );
        }
      } catch (mlErr) {
    ...
    if (!analysis) {
      const kw = keywordFallback(
        extractedText || req.file.originalname || "general report",
      );
      analysis = {
        ...
        labValues: [],
        reasoning:
          "Keyword-based analysis used as primary AI services were unavailable.",
        emergencyAdvice:
          "Please consult a qualified medical professional for accurate interpretation.",
        confidence: 40,
      };
      engine = "keyword";
    }
    ...
    // Sanitize
    if (!VALID_SPECIALITIES.includes(analysis.speciality))
      analysis.speciality = "General physician";
    if (!["LOW", "MEDIUM", "HIGH", "CRITICAL"].includes(analysis.seriousness))
      analysis.seriousness = "MEDIUM";
    analysis.seriousnessScore = Math.max(
      0,
      Math.min(100, Number(analysis.seriousnessScore) || 50),
    );
    ...
    res.json({
      success: false,
      message: error.message || "Report analysis failed. Please try again.",
    });
  }
};
...
const savePrescription = async (req, res) => {
  try {
    ...
    if (userData && doctorData) {
      sendPrescriptionAddedEmail(userData.email, userData.name, doctorData.name);
    }
